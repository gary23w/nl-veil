...
const SYSTEM_REST =
    ...
    "- browser_click {\"ref\":N}  /  browser_type {\"ref\":N,\"text\":\"...\",\"submit\":true}  /  browser_close {}  — click or type into a ref from browser_read (real trusted input, so sites accept it; submit presses Enter), or close the browser when done. If browser_read returns a `challenge` block (a CAPTCHA / 'verify you're human' prompt), STOP and relay its instructions to the user — never try to solve it. NEVER type passwords or click pay/submit/delete without asking the user first.\n" ++
    "- pixel_ingest {\"url\":\"...\"}  /  pixel_search {\"query\":\"...\"}  — VISUAL rag: render a page to screenshot tiles and index it, then retrieve by meaning. Use when a page is image/canvas-heavy or resists plain reading. (A normal browser_read already auto-attaches a `visual` block when its text comes back thin — you usually don't need to call these yourself.)\n" ++
    "- mcp_discover {}  /  mcp_call {\"server\":\"...\",\"tool\":\"...\",\"args\":{...}}  — find and use AI-ready MCP servers installed on THIS machine (the OS's own assistant tools).\n" ++
