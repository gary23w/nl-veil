...
### Branching conversations with a shared family memory
Split a conversation into up to **five tabbed sub-chats** that share **one memory and one workspace**. Chase a
tangent, try a different approach, or run parallel threads of the same build — and the primary plus every branch
**read one family mind**: a discovery in any branch is instantly recallable in all of them, while each
conversation still writes its own partition so the history stays attributable. Branches share the primary's
build tree (files, casts, the Files tab all follow), and every branch turn re-anchors on the primary's live
goal and latest progress. Delete a branch from the tab strip; deleting the primary cascades the family.

### Reliability
- **Delete stops the turn first.** Deleting a conversation now cleanly ends any in-flight turn — an active turn
  runs the Stop routine, a backgrounded turn gets a control-stop — instead of leaving work orphaned.
- **Server-only binaries for every platform actually ship now.** The Linux and macOS standalone
  `veil-server-*` binaries were silently dropped from alpha.2's release (a cross-compile built the GUI it
  couldn't link for those targets); they're back, as true headless server binaries.
- Salvage hardening (a "gutting floor" + structural HTML gates so a recovery pass can't blank a working file),
  plus assorted build-floor and exit-cleanup fixes.

> Data format is unchanged — your `./data` carries forward from alpha.1/alpha.2.
...
### 💬 Chat that builds — now with branches
