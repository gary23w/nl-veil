# the veil — v1.0.0-alpha.10

**The final alpha.** This one is about the first five minutes: getting the app, opening it, and being told
what happened when it can't open.
...
## 🆕 New since alpha.9 — install & first-run fixes

Three fixes, all found by actually downloading a release onto a second machine and watching it fail:

- **A machine with no OpenGL now says so — and keeps working.** On a virtual machine, a Remote Desktop
  session, or a box with a missing/stale graphics driver, there is no OpenGL 3.3, so the desktop window
  cannot be created. Previously the process noticed on its first frame, shut the server down, and exited:
  you double-clicked and *nothing visible happened*. Now it explains the cause in a dialog, points you at the
  web UI, writes the reason to `data/desk-exit-reason.txt`, and — the important part — **leaves the server
  running** so the browser UI still works. (`veil --server-only` remains the deliberate way to run headless.)
- **Windows actually gets a `.zip`.** The download table has always said
  `veil-v1.0.0-windows-x86_64.zip`, but the build only produced a zip when the `zip` tool was present — and
  it isn't on the Windows build machine, so Windows silently shipped a `.tar.gz` that the instructions never
  mentioned. Fixed: Windows now gets the zip the page promises.
- **The app looks like itself on disk.** `veil.exe` and `veil-desk.exe` now carry the veil mark as their icon
  at every shell size (16→256). Explorer, desktop shortcuts, pinned taskbar entries, and the Alt-Tab card no
  longer show the generic executable glyph.
...
>   the web UI (the app now tells you this instead of exiting silently).
