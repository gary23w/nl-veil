...
## 🆕 New since alpha.4 — the edit surface actually lands

A focused reliability pass on the file tools a chat turn drives — the fixes that decide whether an edit to a
real-sized file succeeds or silently falls apart:

- **Large files are editable again from the desk.** `edit_file` tells the model to copy a `42:abc:def` line
  anchor, but only the server drive loop rendered those anchors — the desk's own tool endpoint and the client
  executor handed back plain text, so the surface the desk uses most fell back to fuzzy snippet matching and
  **large files became unfixable**. Every path now sets anchored reads. A windowed read also hashes only the
  window it asked for, so reading part of a big file costs what you requested, not what the file weighs.
- **One shared edit ceiling, no more phantom locks.** The edit limit is now a single 20 MiB constant across
  the tool and the version-control layer. A file that `edit_file` accepted but the commit step couldn't read
  used to fail its internal read *forever* while reporting a transient lock; that mismatch is gone. Edits above
  2 MiB land atomically and keep their history ref, but skip the full snapshot (no writing 20 MiB of history
  for a rollback nobody asked for).
- **Cut-off tool calls no longer vanish.** When a model's tool call arrives truncated, both salvage paths now
  recover the repaired arguments instead of dropping the call.

Carried forward: branching conversations with a shared family memory (sub-chats), the alpha.4 correctness
pass, and standalone `veil-server-*` binaries for every platform.
...
instead of announcing success. **Branch any conversation** into sub-chats that share one memory and workspace.
