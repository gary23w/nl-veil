...
- **The browser extension stays paired across a server restart.** The pairing token is now persisted, so
  restarting the veil no longer forces you to re-pair the extension with your Chrome/Edge — it reconnects on
  its own.

Builds on alpha.8's browser work (text fields visible and confirmable; a click reports its effect) and
alpha.6's extension itself.
