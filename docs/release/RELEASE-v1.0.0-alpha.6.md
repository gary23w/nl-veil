...
A new **browser extension** lets the veil drive the Chrome/Edge you already use — your real profile, logged-in
sessions and all — instead of only its bundled headless Chromium. It can read the console and network activity
of the page and send input without touching the DOM. The headless driver and Pixel RAG capture are still there;
this adds "use the browser I'm already signed into" for tasks that need your real session.

### Smarter by default
- **Advanced reasoning is now the default**, with a **fast mode** to opt back out when you want speed over
  deliberation.
- **Look before planning.** A turn now inspects the actual state before it commits to a plan, and re-checks
  its course mid-turn rather than barreling down a stale one.
- Prose written *after* an answer can no longer accidentally mark a plan subtask complete.

### Desk
- **What you open stays open.** Chat expand/collapse (reasoning traces, tool calls) is now keyed by a stable
  message identity and tracked as a set — an open block no longer snaps shut when a trace is inserted above it
  or the message ring scrolls, and expanding one chip doesn't collapse another. A live reasoning trace
  auto-expands while it streams and folds to a chip only once it's history.

### Correctness & observability
- **`veil doctor`** now reports per-call token spend and your prompt-cache hit rate; the usage ledger rotates
  instead of growing without bound, and its readers no longer report zero after a rotation.
- **Scheduled tasks:** a run that outlives its own interval no longer launches a second copy of itself in
  parallel.
- **Edit surface:** an insert and a replace on the same line no longer clobber each other.
- **HTTP:** a malformed request body is a clean `400`, not a `500`.
- **Crawl:** boilerplate is scored as boilerplate (not silently dropped), and the top-ranked chunk respects the
  caller's byte budget.
- Plus a broad hardening pass with real test verdicts behind the build gate.

> Data format is unchanged — your `./data` carries forward from earlier alphas.
...
>   `NL_ADMIN_PASSWORD`, it **generates a random admin password** and writes it to `data/admin-password.txt`
>   (so a known default can't answer your subnet). Set `NL_ADMIN_PASSWORD` to choose your own, or
>   `NL_BIND=127.0.0.1` to pin the server to this machine only (then the local admin is
>   `admin@neuron-loops.local` / `changeme`, same-machine trust). This program runs code as you — treat an
>   exposed instance accordingly.
> - **The web UI lags the desktop app.** It's still the older swarm console — the richest chat/tasks/settings
>   UX is in the desktop app; the web port to full parity is in progress.
...
it looks before it plans, checks its own work, and repairs failures instead of announcing success. **Branch any
conversation** into sub-chats that share one memory and workspace.
...
reusable **skills**, ingested **documents** (per-doc scopes with across-document recall), a **conversation
family mind** shared across sub-chat branches, and a user model. Recall is relevance-gated and trust-weighted,
and every learned entry is quarantined for your **keep / drop** approval before it binds — the memory can't
quietly poison itself.
...
actually rendered. Or install the **browser extension** and point it at your own Chrome/Edge to work inside your
