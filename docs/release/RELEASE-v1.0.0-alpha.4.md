...
## 🆕 New since alpha.3 — hardening & correctness

Mostly an under-the-hood release: a large test/oracle/invariant pass across the engine, desktop, CLI, and
build, plus a set of real correctness fixes:

- **Memory writes are no longer mis-signaled.** `Mem.observe` returned 0 (failure) for *every* successful
  write, so callers couldn't tell a stored fact from a dropped one — fixed, so memory feedback is truthful.
- **Paste no longer eats messages.** A carriage return in pasted text could swallow a hub broadcast (a JSON
  escaping bug); pastes now round-trip intact.
- **Windows networking is honest about refusals.** A refused connection is treated as a hard failure instead
  of being retried as if transient — faster, clearer failures.
- **Cheaper memory reads.** Reading a single record no longer spawns six neuron subprocesses.
- **Billing correctness.** A model id's capitalisation no longer changes its rate.
- Plus many smaller fixes surfaced by the new test suites (JSON escaping, scan/oracle accuracy, CLI verb
  coverage, deploy workspace bounds, stable-prefix cache invariants).

Carried forward from alpha.3: **branching conversations with a shared family memory** (sub-chats — split a
chat into up to 5 tabbed branches sharing one recall + workspace), and the release now ships the standalone
`veil-server-*` binaries for **every** platform (Linux/macOS/Windows, x86_64 + arm64).
