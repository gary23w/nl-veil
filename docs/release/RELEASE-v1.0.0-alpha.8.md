...
## 🆕 New since alpha.7 — the browser sees more, guesses less

Two improvements to how the AI drives a web page:

- **Text fields are visible now**, not just links and buttons. When the veil reads a page it can see the input
  fields and their current contents, so it can fill a form and *confirm* what it typed instead of firing blind.
- **A click reports its effect.** After a click, the tool tells the agent what actually changed on the page —
  so it stops re-reading the whole page just to guess whether the click worked. Faster, and fewer wasted turns
  fumbling for confirmation.

Together with alpha.6's browser extension (drive your own Chrome/Edge) and `pixel_capture` (snapshot the live
page), the veil can build a web app and then genuinely *use* it — fill the form, click through, and verify
what rendered.
...
**browser extension** and point it at your own Chrome/Edge to work inside your real, logged-in sessions.
