...
- wsock.zig — on Windows, a loopback or IPv4-literal host goes through one blocking Winsock socket under one wall-clock deadline covering connect, send and receive, instead of the Io race (the 2026-09-02 thread-parking fix); a DNS name keeps the portable path (desk/src/wsock.zig is the twin of src/worker/wsock.zig)
