...
Since v1.1.3, marked release installations check GitHub at startup. Settings → Updates includes **App updates**,
...
Settings has five focused pages: General, Models, Connection, Data, and Updates.
Navigation stays fixed while content scrolls; descriptions wrap, and clipped controls
cannot receive pointer input behind the navigation. Notification and token changes
are saved through the worker command queue.

The chat keeps server progress visible above the transcript before the first token.
New-chat starters populate the composer without sending. Bottom-follow retains up to
two lines of spacing through small streaming reflows; manual scroll, folding and
resizing restore current layout bounds. Empty previews do not add phantom rows.
Message measurements invalidate on exact width, text scale, font and conversation
changes. Code headers stay clear of code; tilde and nested fences copy intact.
