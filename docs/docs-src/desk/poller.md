...
- `refresh` (internal) — one poll tick: throttled fleet GET (every ~5s), roster scan, selected-swarm event tail + config + files read, publish-under-lock, then notifyTransitions. A tick that walked the roster also ends, in that same publish, the "deleting..." mark of every delete that landed before it (store.Deleting.walked), and raises "Delete incomplete" for a run the walk still found on disk
...
- `doControl`/`doDeploy`/`doDelete` (internal) — write a control JSON via scan.writeControl, POST a UI-built DeployReq body via netcli.deploy, and delete a swarm the way scan.deleteRoute says: API DELETE for a server run, with an id the server resolves back to that run dir; single-dir deleteTree for a flat CLI run; a notice for a run no server id resolves to. A delete that does not land (unreachable, 401/403, 404, any other status, a failed deleteTree) ends its row's "deleting..." mark before doDelete returns (Store.deleteDone), so the row is clickable again on the UI's next frame
