...
- `doControl`/`doDeploy`/`doDelete` (internal) — write a control JSON via scan.writeControl, POST a UI-built DeployReq body via netcli.deploy, and delete a swarm the way scan.deleteRoute says: API DELETE for a server run, with an id the server resolves back to that run dir; single-dir deleteTree for a flat CLI run; a notice for a run no server id resolves to
