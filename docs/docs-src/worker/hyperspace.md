...
- Eviction is focus-independent: the least-connected (most peripheral) **measured** fact goes, so general skeleton hubs survive. A settle measures every fact's degree; a fact observed since then reads degree 0 because nothing has measured it yet, so it is never the victim while a measured fact remains. Ranked by that 0, each new fact used to be evicted the moment the next one arrived, and a burst of findings between two settles kept only its last.
- With nothing measured (before the first settle, or once a burst has displaced every measured fact) the oldest fact goes, and the oldest also wins a tie. Removal is ordered, so the facts stay in arrival order and index 0 is always the oldest; the `swapRemove` this replaced moved the newest fact into the freed slot, so from the second pre-settle eviction on, a fact that had just arrived went instead. The dedupe key is removed with the victim (a test pins `seen.count == facts.len`), and tests pin the exact survivors of both paths.
