...
- Prune idle runs (`NL_RETENTION_DAYS`, at startup and hourly): a run dir whose own `events.jsonl` is older than the window, unless a running entry holds it in either slash form. A deploy's dir goes whole. A conversation's or scheduled run's build dir loses only the cast's bookkeeping, and its `work/` stays.
- Sweep stranded curl key configs at startup (`sweepKeyScratch`), whatever `NL_RETENTION_DAYS` says. It removes files `llm.isKeyCfgName` names that were last written more than `llm.KEY_CFG_STALE_S` ago (15 minutes, longer than any call runs). It looks at the top level of each run dir, the top of that run's `work/`, and each conversation dir, and nowhere else. Each call deletes its own config, so the sweep takes what a killed process left, plus the per-tag configs earlier builds left in every dir that ever called a hosted model. The age floor keeps it off configs of calls still running, whichever process made them.
...
- `worker/llm` — which file names are curl key configs, and the age past which none is in use (`sweepKeyScratch`)
