...
- Find run dirs only where the server spawns workers: `u{uid}/{spawn id}` (a deploy), `u{uid}/_chat/builds/{conv}` and `u{uid}/_sched/{task}/runs/{stamp}` (a conversation's and a scheduled run's casts). The walk never enters a run's `work/` tree, where a hive's `write_file` can leave a `swarm.json` or `events.jsonl` of its own. A conversation's dir (`u{uid}/_chat/convs/{conv}`) is not a run dir either. Re-adoption, retention and the key sweep use this walk.
- Re-adopt swarms found in the data dir on boot (the "N swarms re-adopted" line at startup), each for the account whose dir holds it. Every run dir gets its own entry, skipped only when an entry already tracks that very dir. The key is the name clients reach the dir by: a scheduled run's conversation id (`scheduled_{task}_{stamp}`, never the stamp every task that ran that minute shares), else the dir's basename. When another dir already holds that key (two accounts can mint one conversation id), the later dir's key gets its account appended (`{name}.u{uid}`).
...
- Prune idle runs (`NL_RETENTION_DAYS`, at startup and hourly): a run dir whose own `events.jsonl` is older than the window, unless a running entry holds it in either slash form. A deploy's dir goes whole. A conversation's or scheduled run's build dir loses only the cast's bookkeeping, and its `work/` stays. That bookkeeping (`cleanCastMeta`, which a stop runs too) includes every curl key config (`llm.isKeyCfgName`, either transport) and the tags' replay copies of request bodies, but never a body a call wrote under its own name (`llm.isCallBodyName`). The conversation's turns and a second cast call models in the same dir, and such a body may be waiting for its curl to read it. A running call's key is never in the dir: curl reads its config from stdin, so every config there is an older build's.
- Sweep stranded call scratch at startup (`sweepKeyScratch`), whatever `NL_RETENTION_DAYS` says. It removes curl key configs (`llm.isKeyCfgName`), and the request bodies and stream sinks each call writes under its own name (`llm.isCallBodyName`), that were last written more than `llm.KEY_CFG_STALE_S` ago (15 minutes, longer than any call runs). It looks at the top level of each run dir, the top of that run's `work/`, and each conversation dir, and nowhere else. No call writes a config now, so every config it finds is an older build's: one a killed call left, or a per-tag one the earliest builds left in every dir that ever called a hosted model. Each call takes its own body and sink along when it returns, so of those the sweep takes what a killed process left. The age floor keeps it off the files of calls still running, whichever process made them, an older build's process on the same data dir included. A tag's replay copy stays.
...
- `worker/llm` — which file names are curl key configs or a call's own request body, and the age past which none is in use (`sweepKeyScratch`, `cleanCastMeta`)
