...
- `syncUser` — one incremental pass: token, bucket check/create, candidate walk, uploads, state write; an upload that finds the bucket gone re-creates it and re-runs the walk from an empty manifest, once per pass
...
- Change detection: `.cf_r2_manifest.jsonl` keeps one `{p, sz, h}` line per uploaded file. The files rewritten whole can keep their length, so they are read and FNV-1a hashed every pass: `context.json`, `brief.json`, `plan.jsonl` (the plan board, overwritten on each status change) and `memories.jsonl` (a FORGET writes the surviving lines back). The rest are compared by size alone. `messages.jsonl` and `digest.jsonl` only append, and `files.jsonl`'s one rewrite compacts it from past 128 KiB down to at most 128 entries, so its size always moves (tested).
...
- Bucket: GET it, else POST to create it. Success, or error code 10004 or 10073 (the name already exists in this account), confirms it; a failure stores Cloudflare's own message — most often R2 not activated. The confirmation is per account: `bucket_ok` holds only while `bucket_account` names the account the token resolves to, so passes skip the check for the same login. A login to another account checks there and, since the manifest described the old account's bucket, starts it over; a state from before `bucket_account` existed is checked once and keeps its manifest. A newly created bucket always starts the manifest over. A failed check clears `bucket_ok` but keeps `bucket_account`, so a later login back to that account resumes.
- A deleted bucket: an upload refused with 10006 NoSuchBucket (or its message) stops the walk, clears `bucket_ok` and empties the manifest, since its objects went with the bucket. The pass then confirms or creates the bucket and walks again, once, so the backup recovers in the pass that noticed. A second "no such bucket" in the same pass is reported and left to the next pass. Stand-in API tests pin both paths.
