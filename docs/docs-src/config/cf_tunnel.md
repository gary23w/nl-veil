...
- `cf_oauth.zig` — `resolveToken` (bearer + account id), `readProfile` (the email Access admits), `apiCall` (curl with the bearer, and each JSON body here, in the config curl reads from its stdin: none of it touches disk), `CF_PROVIDER`
