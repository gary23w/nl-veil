...
pub const State = struct {
    ...
    pub fn parse(self: *State, conn: *HTTPConn, source: anytype) !bool {
        ...
        if (n == 0) {
            // EOF: the peer sent FIN (half/closed the socket). recv() now returns 0 IMMEDIATELY on every call
            // (not a timeout, so SO_RCVTIMEO can't help) — returning `false` made handleRequest's while(true)
            // retry forever and pin the blocking-worker thread at 100% CPU (nl-veil: ~5 such threads spun the
            // server during a watched cast). Signal a closed connection so the worker tears it down instead.
            // NOTE: local patch to vendored httpz — re-apply if the httpz dependency is re-fetched.
