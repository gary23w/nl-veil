...
pub fn isKeyCfgName(name: []const u8) bool {
    for ([_][]const u8{ POST_CFG, STREAM_CFG }) |kind| {
        ...
        if (name.len == kind.len or name[kind.len] == '-') return true;
    }
    return false;
}
...
fn postUrl(gpa: std.mem.Allocator, io: std.Io, run_dir: []const u8, tag: []const u8, url: []const u8, key: []const u8, body: []const u8, local: bool, budget_tokens: u32) Reply {
    ...
    if (run.term != .exited or run.term.exited != 0) {
        ...
        // err() keeps its own copy, so the formatted message is freed here rather than orphaned per failed call.
        const why = std.fmt.allocPrint(gpa, "curl exit: {s}", .{run.stderr[0..@min(run.stderr.len, 200)]}) catch return err(gpa, "curl nonzero exit");
        defer gpa.free(why);
        return err(gpa, why);
    }
...
/// engine and the swarm mind loop check this at their message-build layers (llm.zig cannot heal it — the
/// reasoning text was dropped upstream, not mangled in the body). Case-insensitive; both key phrases sit
/// near the front of the real error.
...
/// turn builder and the swarm mind loop's structured branch). Shared with reasoningSpliceOffset so the
/// reasoning_content splice lands right after `"assistant"`, byte-for-byte in step with the builders —
/// if a builder's serialization changes, this must change with it (the splice searches these exact bytes).
pub const ASSISTANT_OBJ_HEAD = ",{\"role\":\"assistant\"";

/// Where to splice a `,"reasoning_content":...` into a LIVE conversation buffer: right after the head of
/// the most recent assistant object, but ONLY when that object is a tool-call turn that has not already
/// echoed its reasoning. Read from the current buffer (not a saved offset) so mid-loop buffer rewrites
/// (the chat engine's compactWorking) cannot stale it. null ⇒ nothing to heal (no tool-call turn, or
/// already echoed). Pure — tested.
pub fn reasoningSpliceOffset(buf: []const u8) ?usize {
...
pub fn withReasoningEcho(gpa: std.mem.Allocator, conv: []const u8, reasoning: []const u8) ?[]u8 {
    ...
    blk: {
        ...
        out.appendSlice(gpa, conv[prev..]) catch break :blk;
        return out.toOwnedSlice(gpa) catch break :blk;
    }
    out.deinit(gpa);
    return null;
}
...
test "reasoningSpliceOffset finds the tool-call turn head, skips plain + already-echoed turns" {
    ...
    // Already echoed ⇒ no splice (would double it). The leading user turn matters: it gives the assistant
    // object its head comma, so this genuinely exercises the already-carries-it branch (not head-not-found).
    const b2 = "{\"role\":\"user\",\"content\":\"hi\"},{\"role\":\"assistant\",\"reasoning_content\":\"t\",\"tool_calls\":[{}]},{\"role\":\"tool\",\"content\":\"r\"}";
    ...
    // The swarm mind loop's exact serialization (tool_call_id results) splices the same way.
    const b5 = "{\"role\":\"user\",\"content\":\"go\"},{\"role\":\"assistant\",\"content\":\"on it\",\"tool_calls\":[{\"id\":\"a\",\"type\":\"function\",\"function\":{\"name\":\"read_file\",\"arguments\":\"{}\"}}]},{\"role\":\"tool\",\"tool_call_id\":\"a\",\"content\":\"data\"}";
    const off5 = reasoningSpliceOffset(b5).?;
    try std.testing.expectEqualStrings(",\"content\":", b5[off5 .. off5 + 11]);
    // A tool RESULT that mentions the key as escaped text must not read as "already echoed": the JSON
    // string escaping (\" not ") keeps the byte search from matching inside content — pinned here.
    const b6 = "{\"role\":\"user\",\"content\":\"hi\"},{\"role\":\"assistant\",\"content\":\"\",\"tool_calls\":[{}]},{\"role\":\"tool\",\"content\":\"docs say \\\"reasoning_content\\\" is required\"}";
    try std.testing.expect(reasoningSpliceOffset(b6) != null);
}

test "withReasoningEcho splices a dispatch-time copy for a fallback rung, empty reasoning included" {
    const gpa = std.testing.allocator;
...
test "withReasoningEcho heals EVERY bare assistant turn — newest gets the saved reasoning, older the empty echo" {
    ...
    const conv = "{\"role\":\"user\",\"content\":\"go\"}" ++
        ",{\"role\":\"assistant\",\"content\":\"a\",\"tool_calls\":[{\"id\":\"1\"}]},{\"role\":\"tool\",\"tool_call_id\":\"1\",\"content\":\"r1\"}" ++
    ...
    // Mixed buffer: an already-echoed turn is left alone, only the bare one is healed.
    const mixed = "{\"role\":\"user\",\"content\":\"go\"}" ++
        ",{\"role\":\"assistant\",\"reasoning_content\":\"kept\",\"content\":\"a\",\"tool_calls\":[{\"id\":\"1\"}]},{\"role\":\"tool\",\"tool_call_id\":\"1\",\"content\":\"r1\"}" ++
        ",{\"role\":\"assistant\",\"content\":\"b\",\"tool_calls\":[{\"id\":\"2\"}]},{\"role\":\"tool\",\"tool_call_id\":\"2\",\"content\":\"r2\"}";
    const m = withReasoningEcho(gpa, mixed, "t2").?;
    defer gpa.free(m);
    try std.testing.expect(std.mem.indexOf(u8, m, "\"reasoning_content\":\"kept\",\"content\":\"a\"") != null);
    try std.testing.expect(std.mem.indexOf(u8, m, "\"reasoning_content\":\"t2\",\"content\":\"b\"") != null);
...
test "nextMissingEchoOffset walks every bare assistant turn — plain history turns included" {
    ...
    const buf = "{\"role\":\"user\",\"content\":\"go\"}" ++
        ...
        ",{\"role\":\"assistant\",\"reasoning_content\":\"t\",\"content\":\"c\",\"tool_calls\":[{}]},{\"role\":\"tool\",\"content\":\"r\"}" ++
        ",{\"role\":\"assistant\",\"content\":\"d\",\"tool_calls\":[{}]},{\"role\":\"tool\",\"content\":\"r\"}";
...
const SCRATCH_TEST_KEY = "nlk_scratch-test-key-7c1e-not-a-real-credential";

/// Fails if any file in `dir_path` is a curl config or holds a byte of `key`. Returns how many files it read.
fn expectNoKeyOnDisk(gpa: std.mem.Allocator, io: std.Io, dir_path: []const u8, key: []const u8) !usize {
...
test "a hosted call never puts its key on disk: not while curl runs, and not after an answer, an HTTP error, or a dead transfer" {
    ...
    const root = "zig-llm-keyscratch-post-tmp";
    std.Io.Dir.cwd().deleteTree(io, root) catch {};
    defer std.Io.Dir.cwd().deleteTree(io, root) catch {};
    _ = std.Io.Dir.cwd().createDirPathStatus(io, root, .default_dir) catch {};

    const body = "{\"model\":\"keyscratch-model\",\"messages\":[]}";
    const Case = struct { tag: []const u8, reply: []const u8, ok: bool, status: u16 };
    const cases = [_]Case{
        .{ .tag = "answer", .reply = fakehttp.wire("{\"choices\":[{\"message\":{\"content\":\"ok\"}}]}"), .ok = true, .status = 200 },
        .{ .tag = "status", .reply = std.fmt.comptimePrint("HTTP/1.1 500 Internal Server Error\r\nContent-Type: application/json\r\nContent-Length: {d}\r\n\r\n{s}", .{ "{\"error\":\"boom\"}".len, "{\"error\":\"boom\"}" }), .ok = false, .status = 500 },
        // accepted, read, closed with no reply: curl exits nonzero (52) - the exit that never parses a status
        .{ .tag = "dead", .reply = "", .ok = false, .status = 0 },
    };
    for (cases) |c| {
        var srv: fakehttp.Server = undefined;
        ...
        defer gpa.free(kept);
        try std.testing.expectEqualStrings(body, kept);
    }
}
...
test "a streamed call never puts its key on disk: not while curl streams, and not after a clean stream, a failed one that falls back, or a Stop" {
    ...
    const root = "zig-llm-keyscratch-stream-tmp";
    std.Io.Dir.cwd().deleteTree(io, root) catch {};
    defer std.Io.Dir.cwd().deleteTree(io, root) catch {};
    _ = std.Io.Dir.cwd().createDirPathStatus(io, root, .default_dir) catch {};
    ...
    const answer = fakehttp.wire("{\"choices\":[{\"message\":{\"content\":\"fell back\"}}]}");
    const Case = struct { tag: []const u8, stream: []const u8, stop: bool, content: []const u8, conns: u32 };
    const cases = [_]Case{
        .{ .tag = "clean", .stream = fakehttp.wire("data: {\"choices\":[{\"delta\":{\"content\":\"streamed\"}}]}\n\ndata: [DONE]\n\n"), .stop = false, .content = "streamed", .conns = 1 },
        // an error line fails the stream, and complete() - the fallback - answers from the stand-in's second reply
        .{ .tag = "fallback", .stream = fakehttp.wire("data: {\"error\":{\"message\":\"boom\"}}\n\n"), .stop = false, .content = "fell back", .conns = 2 },
        // a chat Stop before a byte arrives: curl is killed mid-start, the partial (empty) reply comes back
        .{ .tag = "stop", .stream = fakehttp.wire("data: [DONE]\n\n"), .stop = true, .content = "", .conns = 0 },
    };
    for (cases) |c| {
        var srv: fakehttp.Server = undefined;
        ...
        try std.testing.expect(step.ok);
        try std.testing.expectEqualStrings(c.content, step.content);
        if (!c.stop) {
            try std.testing.expectEqual(c.conns, srv.conns.load(.monotonic));
            if (std.mem.indexOf(u8, srv.request(), "Authorization: Bearer " ++ SCRATCH_TEST_KEY) == null) {
                std.debug.print("\n[{s}] the stand-in never saw the key: {s}\n", .{ c.tag, srv.request() });
                return error.KeyNeverSent;
            }
        ...
        try std.testing.expect(kept.size > 0);
    }
}
