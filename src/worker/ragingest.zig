...
test "distill: extracts clean sentences from wrapped prose, drops code and short/markup lines" {
    const gpa = t.allocator;
    const text =
        \\# A Heading That Should Be Ignored
        \\
    ...
    // the heading, code, and bullet never became facts
    try t.expect(std.mem.indexOf(u8, body, "Heading") == null);
